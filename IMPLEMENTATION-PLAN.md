...
> 🧭 **M10 — REPORT ARCHITECTURE — DECOMPOSED (2026-07-06; V-AA LARGE classification 2026-07-02 + DQ1-DQ5 mini-chain).** Sub-milestones + sequencing: **M10.A** Vulnerability Endpoints ✅ **CLOSED (2026-07-06;** chain `df68e69` + `5cc968a` + `0251ee7` + P5.A; all γ; no C0; first M10 sub-milestone**)** (Task 10.1; data layer ready: VulnerabilityStatus + VulnerabilityHistory operational — history table now WIRED at PATCH per Q5) → **M10.B** Report Generation + Delivery ✅ **CLOSED (2026-07-26;** chain `92fe1c7` + `f099a5c` + `ac91e62` + `c383b06` + `9b2724e` + `0d7e5e5` + docs micro-commits `67f0676` + `a50898f`; all γ; ADR-034; second M10 sub-milestone**)** (Tasks 10.2+10.3+10.4; report pipeline reachable end-to-end) → **M10.C** Compliance Mapping ✅ **CLOSED (2026-07-31;** chain `bee52c6` + `edea03a` + `40b8243`; all γ; no C0; 3 global tables + first seed migration; third M10 sub-milestone**)** (Task 10.5; 3-table SPEC-canon scope + SOC2/ISO-27001:2013 seed; three-state posture) → **M10.D** Tool Health ✅ **CLOSED (2026-07-31;** chain `74f0688` + `3d94c42`; all γ; no C0/migration/ADR — Redis-read-only over the existing worker heartbeat; fourth + final M10 sub-milestone**)** (Task 10.6; two-state honest availability, no fabricated degraded). No M10.0 foundation needed (R2 + WeasyPrint + models + RLS + test harness all operational; contrast M9).
>
> **🏁 M10 — REPORT ARCHITECTURE — ENTIRELY CLOSED (2026-07-31).** All 4 sub-milestones ✅: **M10.A** Vulnerability Endpoints + **M10.B** Report Generation + Delivery + **M10.C** Compliance Mapping + **M10.D** Tool Health. The DQ1-DQ5 decomposition mini-chain is complete; **DQ5 exhausted — no further M10 trigger.** M10 landed 0 catalogued drifts across all four sub-milestones (cumulative session-tail framing-drift count preserved at 66; 13-instance averted-prediction lineage). Forward-pins carried out of M10: degraded per-tool state → real per-tool health signal; cross-repo tool-name normalization → follow-up; tier-gating → billing (4 instances: /fix + reports + compliance + tool-health); PCI → Enterprise; exhaustive control/CWE coverage → post-launch; ISO 27001:2022 renumber → future migration.
>
...
>
> **Triggers (DQ5), sequential:** ***"Begin M10.A — Vulnerability Endpoints"*** ✅ consumed → ***"Begin M10.B — Report Generation + Delivery"*** ✅ consumed → ***"Begin M10.C — Compliance Mapping"*** ✅ consumed → ***"Begin M10.D — Tool Health"*** ✅ consumed. **DQ5 EXHAUSTED — M10 entirely closed; no further M10 trigger.**
>
...
> **🔒 Task 10.5 — CLOSED at M10.C (2026-07-31).** Landed as `models/compliance.py` (3 tables) + `services/compliance/frameworks.py` (inline SOC2 + ISO 27001:2013 seed + `seed_compliance_data()` runtime helper) + `services/compliance/mapper.py` (three-state posture) + `routes/compliance.py` (3 GETs) + the seed migration `a7c3e9f04b21` + `tests/models/test_compliance.py` + `tests/routes/test_compliance.py`. **Scope-delta resolved in canon's favor:** SPEC §DB has **3** tables (`compliance_frameworks` → `compliance_controls` → `cwe_control_mappings`) vs the 2 named below — the normalized `compliance_controls` middle layer was added (echoes M10.A 7-vs-4 + M10.B 5-vs-4). These are **GLOBAL reference data** (SPEC line 345) — no TenantMixin, no RLS, **not in TENANT_TABLES** (the deliberate inverse of M10.B's reports table; test-enforced ∅ intersection). Posture is **three-state** — pass / fail / **not_assessed** (unmappable controls are never overclaimed as pass). First `op.bulk_insert` data/seed migration in the repo. PCI omitted at MVP (Enterprise-tier); tier-enforcement forward-pinned to billing. All endpoints ungated GETs (no audit per V-JJH). Full lock-record: `plans/2026-07-27-m10c-compliance-mapping-design.md`.
...
> **🔒 Task 10.6 — CLOSED at M10.D (2026-07-31).** Landed as `routes/tools.py` (`GET /orgs/{org_id}/tools/health`) + `services/tool_health.py` (the roll-up aggregator) + `tests/routes/test_tools.py`. **No C0, no migration, no ADR** — a Redis-read-only endpoint over an EXISTING signal (the leanest sub-milestone in the arc). **Read-and-aggregate over the engine worker heartbeat** (`shieldscan:workers:*` via `redis.scan_iter` — first `scan_iter` in the api, no new dep; **no engine change, no signal written**). **Two-state honest availability** — a tool is `available` if ≥1 live worker advertises it (via the heartbeat `engines` list), `unavailable` if none; **NO fabricated "degraded"** (the heartbeat carries liveness + engines-list, not per-tool health — the direct echo of M10.C's `not_assessed` honesty; degraded forward-pinned to a real per-tool signal). Liveness = SETEX-TTL presence (crash → key expires ≤60s). Org-scoped/membership-gated but **GLOBAL fleet data** (workers serve all tenants — auth-gating, not data-partitioning; the M10.C global-behind-org-path shape); distinct from `/ready` (API-pod infra vs worker-fleet). Ungated GET, no audit (V-JJH). Two C1 grounding findings: **V-MMD** drain-status is NOT a live signal (the `HSET worker:$id status "draining"` is ops-runbook-only, not written by worker code — drain-awareness forward-pinned, test skipped-and-noted); the **13th averted-prediction** — a cross-repo tool-name mismatch (engine `registry.Engines()` vs the api's `SCAN_TYPE_TOOLS` vocabulary; known-set reused from `orchestrator.py` single-source; normalization forward-pinned). Suite 834 passed / 1 skipped, ZERO regressions. Full lock-record: `plans/2026-07-31-m10d-tool-health-design.md`. **The plan-literal "healthy/degraded/unhealthy" three-state below is resolved to two-state per the honesty lock.**
