...
## 10. Phase 5.A Drift Annotations (2026-05-24; Stage 3 implementation drifts beyond pre-verification catches)

Following Stage 3 cross-repo implementation (docs `9a57865` → api `742faed` → engine `b48fef8`), the following drifts surfaced between this design doc canonical authority (commit `b344d0c`) + plan (`00dd2d1`) and empirical implementation reality. Annotations preserve original design doc text per historical-authority discipline; this section documents post-Stage-3 reality. Mirrors Task 7.6 Phase 5.A `b187d63` annotation shape. Cumulative session-tail framing-drift count: **44 catches** across Task 7.5d + 7.1 + 7.4 + 7.5e + 7.6 + ADR-015 lifecycle arc.

**Drift #40 — Y-AUDIT (b) lock; favorable infrastructure-discovery (pre-verification).** Plan §3.5 + plan §C2.3 framed Y-AUDIT default as "(a) new `credential_access_audit` table". Stage 3 Commit 2 pre-verification surfaced existing audit infrastructure (`src/app/models/audit.py` `AuditLog` model + `src/app/services/audit.py` `audit()` helper + `ScanAction` enum with reserved M5+ slots + RLS already enabled). Y-AUDIT locked at execution at **(b) extend existing**: added `ScanAction.SCAN_CREDENTIAL_DECRYPTED = "scan.credential_decrypted"` enum value; emission via existing `audit()` helper with `details={"credential_id", "credential_auth_type"}`. Zero migrations needed (saved C2.3 entirely); inherits RLS + existing query/SIEM tooling; single audit truth source preserved; matches reserved-enum-slot extension precedent per DRIFT-LOG 2026-04-30 ScanAction-split entry.

**Drift #41 — Credential lookup architectural correction (CRITICAL static; pre-verification).** Plan §3.1 + design doc §3.1 pseudocode used `scan.credential_id` as the FK lookup field. Stage 3 Commit 2 pre-verification surfaced: **`Scan` model has only `project_id` field; NO `credential_id` FK exists.** `ProjectCredential` has `project_id` FK with `unique=True` per M1 invariant + Task 3.X migration `d4f6b1e9a527` ("exactly one credential per project"). Refined credential lookup: `SELECT ProjectCredential WHERE project_id = scan.project_id; scalar_one_or_none` (returns 0-or-1 row). **Pre-verification caught architectural assumption mismatch before any code written** — design doc + plan pseudocode was architecturally wrong from the start. Strongest verify-then-draft validation this multi-session arc.

**Drift #42 — Audit-emission-site refactor (pre-verification).** Plan §C2.2 framed audit emission inside `_build_job_payload`. Pre-verification surfaced: Q6 (a) audit row requires `dispatcher_user_id` but `_build_job_payload(scan, job)` doesn't receive identity. Refactored credential-decrypt + audit emission INTO `dispatch()` (after existing `SCAN_DISPATCHED` audit + before per-job loop). `_build_job_payload(scan, job, auth_payload)` signature extends with `auth_payload` parameter (pre-built dict consumed). Single-decrypt + single-audit per dispatch; multi-job dispatches reuse same `auth_payload`. Cleaner ScanAction-policy compliance ("scan-level only, never per-job").

**Drift #43 — Latent pre-existing regression (caught at Stage 3 Commit 2 pytest gate).** `tests/routes/test_scans.py` `test_create_api_scan_creates_3_jobs` asserted 3 jobs but `SCAN_TYPE_TOOLS[ScanType.API]` now has 4 entries (`sqlmap` appended in Task 7.6 cross-repo pair Commit 1, `2cd4065`). Test wasn't updated in `2cd4065`. Latent regression blocked pytest gate during Stage 3 Commit 2 verification. 2-line defensive inline fix (rename test + assert 4). Surfaces verification-gate methodology drift at Task 7.6 Phase 3 Commit 1 (bounded direct-impact diff inspection; full-suite pytest not run pre-commit).

**Drift #44 — Integration test network topology mismatch (CRITICAL dynamic; caught at Stage 3 Commit 3 integration test execution post-cookie-threading).** SQLMap pool containers run on default Docker bridge network; `http://localhost:18080` inside SQLMap container resolves to the SQLMap container's own loopback, NOT the DVWA host-port binding on the test host's `127.0.0.1:18080`. Phase 1 wiring-validation test (engine commit `723426d`) **masked** the issue by accepting "0 findings" as v1 Q7 (b)-scoped behavior. Drift #35 closure mechanism's V4 baseline assertion upgrade surfaced the underlying issue empirically (first run with cookie wiring still yielded 0 findings despite operational argv shape). Manual debug via `--network=host` SQLMap docker run reproduced V4 baseline exactly (4 sql_injection + DBMS fingerprint), confirming argv format correct + cookie semantics valid + only network topology at fault. **Fix:** `bootstrapDVWA` extended to `ContainerInspect` DVWA post-start + return bridge IP (additional return value); SQLMap target URL uses `http://<dvwa-bridge-ip>:80/...` rather than `http://localhost:18080/...`. Host-port 18080 retained for host-side bootstrap httpClient only. Foundational architectural drift previously masked by wiring-validation assertion permissiveness.

**Catch-mode complementarity validated.** Drift #41 (static; pre-code; verify-then-draft discipline) + Drift #44 (dynamic; post-code; empirical-assertion discipline) demonstrate **complete coverage across both axes** of the verify-then-implement methodology. Static analysis caught the architectural assumption mismatch before any code was written; dynamic empirical assertion caught the network-topology issue invisible to static analysis. Together they prove the discipline's load-bearing value across the entire static↔dynamic continuum. Wiring-validation reframing (Task 7.6 Drift #35) was the precondition that enabled the V4 baseline upgrade to surface Drift #44 — a forward-pin's closure mechanism doubled as a latent-defect detector.
