...
### ADR-035: RLS Enforcement — connect-as-app-role, null-safe predicate, tenant guard

**Status:** Accepted (2026-08-15; landed across Phase 2 `637f694`, Phase 3a `c36d841`, Phase 3b + Phase 4 `51f7a5e`, post-cutover fix `5a88d0f`, all shieldscan-api).

**Related:** ADR-012 (`api_keys` credential-indexed carve-out — the one table deliberately outside RLS), ADR-013 (Python sole-writer; the Go worker holds no PostgreSQL access, so only the api's connection identity changes), ADR-029/030/031/032 (the AI pipeline whose flush-per-vulnerability pattern constrains the design), `dbdccfd` (the originating commit), `db/policies.py`, `db/tenant_context.py`, `db/tenant_guard.py`, `tests/test_rls_invariants.py`.

**Context.** `dbdccfd` enabled and FORCED row-level security on 13 tenant tables with a `tenant_isolation` policy gated on the session GUC `app.current_org_id`, and provisioned a `shieldscan_app` role as `NOSUPERUSER NOBYPASSRLS NOINHERIT`. Its commit message states: *"The app connects / tests SET ROLE to this. Owner role `shieldscan` stays reserved for migrations and admin ops."* Its diff touched only `db/policies.py` and `db/__init__.py`. The app half was never written — `git log -S "SET ROLE" -- src/` returns that commit and the only line it adds to `src/` is a **comment**.

The consequence ran for four months: the api connected as `shieldscan`, a SUPERUSER with BYPASSRLS, which bypasses RLS unconditionally (FORCE does not override it). Every policy was inert in production. Tenant isolation rested entirely on application-layer `WHERE organization_id = ...` clauses — which a route sweep found to be complete, so there was no live cross-tenant leak, but it was a single layer where two were designed, and several internal services scope by `scan_id` alone with comments explicitly citing RLS as the backstop.

The reason it survived is the part worth recording: **the test fixture did `SET ROLE shieldscan_app`, so the suite exercised the enforced path and reported tenant isolation as verified while production took the bypassed one.** Nothing could contradict the commit message. The defect was not a missing `SET ROLE`; it was the absence of anything able to detect its absence.

**Decision.** Five decisions, each established by demonstration against a real non-superuser role and a real RLS'd table rather than by reasoning. The probe scripts are named per decision; they are evidence, not deliverables, and live in the session scratchpad.

- **D1 — Connect AS the app role, not `SET ROLE` from the owner.** `SET ROLE` is connection-scoped and was demonstrated to survive pool check-in: a second session on the same pooled connection still reported `current_user = shieldscan_app`. The inverse is what condemns it — pooled connections stay **superuser** for anything that does not opt in, so it fails **open** for a consumer, a script, or a future `get_db` variant. Connecting as the role fails closed by default and collapses the pooling hazard from two dimensions to one (the GUC). A `SET ROLE` from a superuser connection is also reversible by `RESET ROLE`. The role gained `LOGIN` by migration; its password is set out-of-band, never in a migration. Migrations keep the owner via a separate `MIGRATION_DATABASE_URL` — they do DDL and `CREATE ROLE`, which the app role cannot and must not do. *(`rls_pool_probe.py`)*

- **D2 — Policy predicate is `organization_id = NULLIF(current_setting('app.current_org_id', true), '')::uuid`.** All three parts are load-bearing. The one-argument form **raises** `unrecognized configuration parameter` when the GUC was never set, turning a missed tenant context into a hard 500. The two-argument form alone is **not sufficient**: once the parameter has been touched in a session, clearing it — by `RESET`, or by a `SET LOCAL` reverting at commit — leaves an **empty string, not NULL**, and `''::uuid` raises `invalid input syntax for type uuid`. Those are precisely the states a connection-reset handler and a mid-request commit produce, so the bare two-argument form would have preserved the 500 in exactly the cases the change exists to eliminate. With `NULLIF`, every state degrades to **zero rows and no error**: a missed tenant context can never mean "all rows", and never means a 500. *(`rls_guc_repr_probe.py`)*

- **D3 — Pool reset handler issues `ROLLBACK` then `RESET`, in that order.** A non-`LOCAL` `SET` lives on the connection and survives the pool's default rollback-on-return, so a request that commits leaks its org to the next request that checks that connection out — demonstrated: a second session read the first session's tenant rows. Ordering is not stylistic: `SET`/`RESET` are themselves transactional, so a bare `RESET` is undone by the pool's subsequent `ROLLBACK` and the context leaks anyway. `ROLLBACK`-then-`RESET` was the only variant clean across all three cases tested (clean check-in, check-in with an open transaction, sustained churn). `DISCARD ALL` also works but is far broader — it drops prepared statements, plans and temp state — and raised `ActiveSQLTransactionError` on one path. *(`rls_reset_probe.py`)*

- **D4 — Tenant context is re-applied per transaction from `session.info`, by one setter.** A Session releases its connection at every commit and checks out a fresh one, so with the reset handler installed a request that commits and then reads sees **zero rows** — the `routes/projects.py` commit-then-read-credentials pattern, which calls `scalar_one()` and would raise `NoResultFound`. `TenantSession.after_begin` re-applies from `session.info`, tying context to the logical request rather than to whichever connection serves it, with no connection pinning. `SET LOCAL` is ruled out: it survives the AI pipeline's flush-per-vulnerability but is cleared by commit. All 13 GUC-setting call sites collapse to `set_tenant_context`, which writes intent **and** applies the GUC — under D4 a site that writes the SET but not the intent works until the request's first commit and then silently loses context, a subtler failure than never setting it. The statement uses `set_config(name, value, is_local)` with a real bind parameter; PostgreSQL rejects a bind parameter on a bare `SET`. *(`rls_rebind_probe.py`, `rls_consumer_probe.py`)*

- **D5 — The guard checks intent AND applied AND that they match.** With D2 a missing context yields zero rows rather than an error — the right *safety* posture, a terrible *detectability* one for a vulnerability scanner, where "0 vulnerabilities found" is a false negative a customer acts on while a 500 gets investigated. `MissingTenantContextError` on `do_orm_execute` converts silence into a distinct, logged failure. It validates two independent facts: *intent* (`session.info`) and *applied* (`connection.info`, written only after the statement lands). A guard trusting `session.info` alone was demonstrated to **wave through** a statement whose GUC was never applied — the exact silent-empty result it exists to catch. No `current_setting` round-trip: the hook fires on every ORM-mediated statement, so table identification reads `ORMExecuteState.all_mappers` and both facts are in-memory dicts. *(`rls_guard_probe.py`)*

**Guard coverage, and why writes are covered elsewhere.** `do_orm_execute` fires for everything routed through `Session.execute()` — ORM selects and Core `select()`/`update()`/`delete()`, all verified guarded — but **not** for unit-of-work flush. The flush-generated statements are covered by other mechanisms, verified individually:

| operation | caught by | behaviour |
|---|---|---|
| SELECT (any, via `execute`) | the guard | raises `MissingTenantContextError` |
| Core `UPDATE`/`DELETE` via `execute` | the guard | raises |
| `INSERT` (`add` + flush) | policy `WITH CHECK` | raises RLS violation |
| `UPDATE` (mutate + flush) | SQLAlchemy rowcount check | raises `StaleDataError` |
| **`DELETE` (`delete` + flush)** | **nothing** | **`SAWarning` only; flush and commit both succeed** |

**Known silent path — ORM delete via flush.** `session.delete(obj)` + `flush()` with no tenant context matches zero rows and SQLAlchemy only warns (`confirm_deleted_rows` warns, it does not raise). The row is **not** deleted and no other tenant is touched — no data loss, no cross-tenant effect — but the caller is told a delete succeeded that did not, which in an API means a 204 on a resource that still exists. Reachability is narrow: obtaining the object requires a load, which is a guarded SELECT that would already have raised, and nothing in the request path clears `session.info` mid-request. Documented rather than fixed because closing it needs a `before_flush` hook over `session.deleted`, a behavioural change deserving its own review. *If a delete endpoint ever reports success against a row that still exists, look here first.* *(`rls_update_delete_probe.py`)*

**Kill-switch is guard-scoped only.** `TENANT_GUARD_ENABLED` (default on, `WARNING` at every startup when off, test asserting the default) governs **the guard and nothing else**. It deliberately does not govern `after_begin` or the reset handler: their off-state was demonstrated to leak a committed GUC to the next request, so a single flag over all three would have an off-state that opens a cross-tenant leak — strictly worse than the failure it is meant to recover from. One runtime switch, one 2am decision, and no way to create a leak by flipping it. The reset handler is **not** required for the guard's soundness (a stale marker from a different org gives `match=False`; from the same org the connection GUC genuinely still holds that org) — it earns its place as the defence that survives the guard being switched off. *(`rls_coupling_probe.py`)*

**⚠ Pre-flight trap — a cutover pre-flight must run with the post-cutover config.** The pre-flight for this work — full suite green with sessions connected as `shieldscan_app` and the guard on — passed at 870, and was then invalidated by the cutover itself: `conftest` derived its DDL url from `DATABASE_URL`, so the moment that flipped to the app role, `drop_all` failed with *must be owner of table*. **The pre-flight ran with `DATABASE_URL` still pointing at the owner, so the one code path that differs only after the flip was never exercised.** The result was accurate but incomplete, and reporting it as full coverage was wrong. This is the same shape as the defect this ADR remediates — a check that passes because it never takes production's path — and is its sixth instance in one week. **A pre-flight that validates a cutover must run under the post-cutover configuration, or it proves nothing about the paths that only differ after the flip.** Fixed in `5a88d0f` by deriving the DDL url from `MIGRATION_DATABASE_URL`, correct in both directions.

**Consequences.** RLS is enforced in production for the first time: the api connects as a role that is neither SUPERUSER nor BYPASSRLS, so all 13 policies are evaluated and the app-layer `WHERE organization_id` clauses return to being defence-in-depth. The Go worker is untouched (ADR-013). Rollback is two switches and no database change — revert `DATABASE_URL` to the owner and set `TENANT_GUARD_ENABLED=false`; Phase 2's `NULLIF` predicate and the `LOGIN` grant are inert under a superuser connection, which is why Phase 2 could land days ahead of the cutover.

**Forcing functions (the actual remedy).** Five invariant tests in `tests/test_rls_invariants.py` exist because the original defect was undetectable, not because it was subtle: the app's role is neither SUPERUSER nor BYPASSRLS (**the assertion that would have contradicted `dbdccfd` on day one**); no engine-level `.connect()`/`.begin()` in `src/`; no `text()` statement names a tenant table; no `SET app.current_org_id` outside the setter; no session listeners registered globally on `orm.Session`. Two were negative-tested rather than assumed — and the global-listener test **passed with a listener registered on `orm.Session`**, i.e. was toothless, before being rewritten against `dispatch._clslevel`. `conftest` now connects its sessions as `shieldscan_app` rather than simulating it via `SET ROLE`, which is what removes the test/production divergence permanently.

#### Forward-pins

- `before_flush` closure of the ORM-delete silent path → own review (behavioural change).
- `api_keys` RLS carve-out under a non-superuser role — the app role can read every org's rows, app-layer scoping is the only boundary → design review (ADR-012 amendment if narrowed).
- Multi-org sessions (a user with memberships in >1 org) → the setter and `require_org_membership` already re-set per request; the zero-org audit skip would need revisiting.
- Retiring the app-layer `WHERE organization_id` clauses now that RLS is enforced → NOT pinned; defence-in-depth is deliberate and both layers stay.
